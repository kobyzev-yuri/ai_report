...
SELECT 
    ...
    -- с конвертацией УЕ->руб по курсу BM_CURRENCY_RATE (currency_id=4): последняя запись с START_TIME <= конец календарного месяца OPEN_DATE (курсы в таблице — обычно на последний день месяца; не конец месяца отчётного периода СФ).
    ...
    -- Разовый платёж подключения, учтённый в доходах (рубли, курс УЕ на конец календарного месяца OPEN_DATE; месяц OPEN_DATE = месяц отчётного периода)
    MAX(
        CASE
            WHEN tsp.TARIFF_SINGLE_PAYMENT_MONEY IS NULL THEN NULL
            WHEN sa.OPEN_DATE IS NULL THEN NULL
...
-- Курс УЕ для месяца OPEN_DATE: BM_CURRENCY_RATE.START_TIME обычно последний день месяца; сравнение с TRUNC(OPEN_DATE) отрезало бы курс своего месяца — берём LAST_DAY(TRUNC(OPEN_DATE,'MM')).
LEFT JOIN (
    SELECT
        START_TIME,
        MAX(RATE) AS RATE
    ...
    ON cr.START_TIME = (
        ...
        WHERE r.CURRENCY_ID = 4
          AND r.DOMAIN_ID = 1
          AND r.START_TIME <= LAST_DAY(TRUNC(sa.OPEN_DATE, 'MM'))
    )
GROUP BY 
    ms.SERVICE_ID,
    sa.OPEN_DATE,
