...
**Голосовой диалог с ассистентами (тест):** подпроект `voice_chat/` — минимальный Flask-интерфейс с логином портала, диалогом (биллинг + спутниковый) и записью голоса → транскрипция. Запуск из корня: `python -m voice_chat.app` (порт 5001). Подробнее: `voice_chat/README.md`.
