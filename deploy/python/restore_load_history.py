...
на основе данных, уже загруженных в таблицы SPNET_TRAFFIC и STECCOM_EXPENSES (Oracle)

Использование:
    python restore_load_history.py [--dry-run] [--check-only]
"""
...
def get_oracle_config():
    """Получение конфигурации Oracle из переменных окружения"""
...
def check_missing_logs():
    """Проверка файлов, загруженных в таблицы, но отсутствующих в load_logs (Oracle)"""
    config = get_oracle_config()
...
def main():
    parser = argparse.ArgumentParser(
        description='Восстановление истории загрузок в load_logs (Oracle)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
