...
- `deploy/` - директория для деплоя, **генерируется из корня** скриптом `prepare_deployment.sh`
  - не редактировать вручную — любые изменения сотрутся при следующем запуске `prepare_deployment.sh`
  - `sync_deploy.sh` - скрипт синхронизации (берёт файлы из `deploy/` и кладёт их на сервер)

### Рабочая директория vs deploy

- **Рабочая директория:** корень проекта `~/ai_report`
  - здесь редактируются исходники (`tabs/*`, `kb_billing/*`, `oracle/*`, `python/*`, `streamlit_*.py`, скрипты и т.д.)
  - именно этот код считается source of truth
- **Директория `deploy/`:**
  - это минимальный “пакет для выкладки” (артефакт сборки)
  - всегда пересобирается командой:
    ```bash
    cd ~/ai_report
    ./prepare_deployment.sh
    ```
  - содержимое `deploy/` нельзя править руками — изменения нужно вносить в корень и затем снова вызывать `./prepare_deployment.sh`

### Типичный workflow синхронизации на сервер

1. **Правка кода / KB** в корне `~/ai_report`.
2. **Сборка deploy-пакета**:
   ```bash
   cd ~/ai_report
   ./prepare_deployment.sh
   ```
3. **Синхронизация на сервер** (код, SQL, KB-файлы):
   ```bash
   cd deploy
   SSH_CMD="ssh -p 1194" ./sync_deploy.sh root@82.114.2.2
   ```
4. **Если менялась KB** — обновление коллекции в Qdrant (опционально, но обычно нужно):
   ```bash
   cd ~/ai_report
