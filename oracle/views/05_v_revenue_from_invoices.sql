...
SELECT 
    -- Информационные колонки (как в затратах); для UI/экспорта см. явный список в get_revenue_report (utils/queries.py)
    ms.SERVICE_ID,
    ...
    -- с конвертацией УЕ->руб по курсу BM_CURRENCY_RATE (currency_id=4): строка курса с TRUNC(START_TIME) = TRUNC(sa.OPEN_DATE) (день подключения), не период СФ и без LAST_DAY.
    ...
    -- Разовый платёж подключения, учтённый в доходах (рубли, курс УЕ: BM_CURRENCY_RATE на календарный день OPEN_DATE sa; месяц OPEN_DATE = месяц отчётного периода)
    MAX(
        CASE
            WHEN tsp.TARIFF_SINGLE_PAYMENT_MONEY IS NULL THEN NULL
            WHEN sa.OPEN_DATE IS NULL THEN NULL
...
-- Курс УЕ по календарному дню OPEN_DATE (sa): один проход по BM_CURRENCY_RATE + JOIN по дню (без OUTER APPLY — иначе подзапрос на каждую строку bc до GROUP BY и отчёт «висит»).
LEFT JOIN (
    SELECT
        TRUNC(r.START_TIME) AS rate_day,
        MAX(r.RATE) KEEP (DENSE_RANK LAST ORDER BY r.START_TIME, r.RATE_ID) AS RATE
    FROM BM_CURRENCY_RATE r
    WHERE r.CURRENCY_ID = 4
      AND r.DOMAIN_ID = 1
    GROUP BY TRUNC(r.START_TIME)
) cr ON cr.rate_day = TRUNC(sa.OPEN_DATE)
GROUP BY 
    ms.SERVICE_ID,
    sa.OPEN_DATE,
    bc.BASE_CONTRACT_ID,
...
/
COMMENT ON COLUMN V_REVENUE_FROM_INVOICES.SUB_CONTRACT_ID IS 'SUB-XXXXX из V_IRIDIUM_SERVICES_INFO по (ACCOUNT_ID+IMEI) или по CONTRACT_ID; удобно, когда CONTRACT_ID=IMEI'
/
COMMENT ON COLUMN V_REVENUE_FROM_INVOICES.CUSTOMER_NAME IS 'Имя клиента (агрегат из V_IRIDIUM_SERVICES_INFO); для сопоставления с V_CONSOLIDATED_REPORT_WITH_BILLING.CUSTOMER_NAME'
/
...
/
COMMENT ON COLUMN V_REVENUE_FROM_INVOICES.PERIOD_YYYYMM IS 'Финансовый период YYYY-MM (из BM_PERIOD) для JOIN и фильтров'
/
COMMENT ON COLUMN V_REVENUE_FROM_INVOICES.PERIOD_MONTH_NAME IS 'Название месяца из BM_PERIOD (например, Янв, Фев)'
/
...
/
COMMENT ON COLUMN V_REVENUE_FROM_INVOICES.CURRENCY_MNEMONIC IS 'Мнемоника валюты счёта-фактуры из BM_CURRENCY (RUR). В отчёте Streamlit колонка выводится как «тарифная валюта» того же смысла.'
/
COMMENT ON COLUMN V_REVENUE_FROM_INVOICES.ACC_CURRENCY_ID IS 'ID валюты лицевого счета (1 = рубли, 4 = УЕ); для JOIN и фильтров'
/
