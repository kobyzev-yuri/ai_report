...
WHERE IMEI = '&imei'
  AND TO_CHAR(TO_DATE(TO_CHAR(BILL_MONTH), 'YYYYMM'), 'YYYY-MM') = '&period_yyyymm';
...
WHERE v.IMEI = '&imei'
  AND TO_CHAR(TO_DATE(TO_CHAR(v.BILL_MONTH), 'YYYYMM'), 'YYYY-MM') = '&period_yyyymm';

PROMPT === 3) Новый фильтр (LEFT JOIN + EXISTS по любой активной услуге с начислениями в периоде) ===
SELECT v.SERVICE_ID, v.CONTRACT_ID, v.IMEI,
       TO_CHAR(TO_DATE(TO_CHAR(v.BILL_MONTH), 'YYYYMM'), 'YYYY-MM') AS PERIOD_YYYYMM,
       v.REVENUE_SUSPEND_ABON, v.REVENUE_TOTAL
