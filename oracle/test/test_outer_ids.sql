...
SELECT 
    ...
    (SELECT oi.EXT_ID FROM OUTER_IDS oi 
     WHERE oi.ID = c.CUSTOMER_ID AND oi.TBL = 'CUSTOMERS' AND ROWNUM = 1) AS CODE_1C_FROM_VIEW,
...
PROMPT ============================================================================
SELECT DISTINCT TBL, COUNT(*) AS CNT
FROM OUTER_IDS
GROUP BY TBL
ORDER BY TBL;
...
FROM SERVICES s
JOIN CUSTOMERS c ON s.CUSTOMER_ID = c.CUSTOMER_ID
LEFT JOIN OUTER_IDS oi ON oi.ID = c.CUSTOMER_ID AND oi.TBL = 'CUSTOMERS'
WHERE s.TYPE_ID IN (9002, 9014);
