-- Проверка данных в steccom_advance_charge_by_period_raw для октября
SET PAGESIZE 0
SET FEEDBACK OFF
SET HEADING ON
SET LINESIZE 2000
...
steccom_advance_charge_by_period_raw AS (
    SELECT CONTRACT_ID, imei, transaction_month, fee_advance_charge, 'SAME_MONTH' AS source_type FROM same_month
    UNION ALL
    SELECT CONTRACT_ID, imei, transaction_month, fee_advance_charge, 'NEXT_MONTH' AS source_type FROM next_month
),
...
-- Проверка: IMEI с несколькими источниками (SAME_MONTH и NEXT_MONTH)
SELECT 
...
WHERE transaction_month = '202510'
  AND source_count > 1
ORDER BY fee_advance_charge DESC
